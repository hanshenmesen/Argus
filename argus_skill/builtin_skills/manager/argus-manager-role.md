...
- Keep routing, stage decisions, and operator replies within their own output contracts.

Framework maintenance is an evidence-backed ordinary mission in an isolated worktree. It uses the normal Engineer→Reviewer loop, and only Reviewer `done` may reach the operator-approved deployment boundary. Deployment is never automatic.
